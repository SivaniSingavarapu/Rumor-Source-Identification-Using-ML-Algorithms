...
def prediction():
    if request.method == 'POST':
        ...
        if prediction == 1:
            ...
            return render_template('user/prediction.html', msg=msg, suggestion=suggestion)
        else:
            msg = 'RUMOUR'
            suggestion = (
...
def feedback():
    if request.method == 'POST':
        ...
        sql = "INSERT INTO feedbacks (user_name, user_email, rating, comment) VALUES (%s, %s, %s, %s)"
        val = (user_name, user_email, rating, comment)
        mycur.execute(sql, val)
        mydb.commit()
       
        return render_template('user/feedback.html', msg = "Thanks for your feedback!")
...
def view_feedbacks():
    ...
    mycur.execute(sql)
    data = mycur.fetchall()
    return render_template('admin/view_feedbacks.html', data = data)
